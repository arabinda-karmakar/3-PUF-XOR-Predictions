...
################################
	...
	X = np.c_[X, np.ones(n)]
	X_t = np.transpose(X)
	X_n = khatri_rao(X_t, X_t)
	X_new = np.transpose(khatri_rao(X_n, X_t))
...
################################
	...
	lambda_param = 0.003
	y_renamed = get_renamed_labels(y)
...
################################
		for i in range(len(X_new)):
			if ((y_renamed[i]*(np.dot(np.transpose(X_new[i]), W))) >= 1):
				W -= learning_rate * (2 * lambda_param * W)
			else:
				W -= learning_rate * (2 * lambda_param * W - np.dot(X_new[i], y_renamed[i]))
				# B -= learning_rate * y_renamed[i]
